document.addEventListener('DOMContentLoaded', () => {
    ...
    const UI = {
        ...
        filterToday: document.getElementById('filter-today'), filterHideCompleted: document.getElementById('filter-hide-completed'),
        categoryTabs: document.querySelectorAll('.tab'), toggleCompletedBtn: document.getElementById('toggle-completed'),
        inputs: {
    ...
    const state = { search: '', category: 'all', onlyToday: false, hideCompleted: false };
    ...
    if(UI.filterToday) UI.filterToday.addEventListener('change', (e) => { state.onlyToday = e.target.checked; renderList(); });
    if(UI.filterHideCompleted) UI.filterHideCompleted.addEventListener('change', (e) => { state.hideCompleted = e.target.checked; renderList(); });
    ...
    if(UI.toggleCompletedBtn) {
        UI.toggleCompletedBtn.addEventListener('click', () => { 
            UI.toggleCompletedBtn.classList.toggle('expanded'); 
            UI.list.completed.classList.toggle('collapsed'); 
        });
    }
    ...
    const renderList = async () => {
        ...
        tasks = tasks.filter(t => {
            if (state.search && !t.title.toLowerCase().includes(state.search) && !(t.description && t.description.toLowerCase().includes(state.search))) return false;
            if (state.category !== 'all' && t.categoryId !== state.category) return false;
            if (state.hideCompleted && t.completed) return false;
            if (state.onlyToday && (!t.dueDate || new Date(t.dueDate).toLocaleDateString('en-CA') !== todayStr)) return false;
            return true;
        });

        UI.list.active.innerHTML = ''; UI.list.completed.innerHTML = ''; let cCount = 0;
        ...
        tasks.forEach(task => {
            ...
            if (task.completed) { UI.list.completed.appendChild(li); cCount++; } else UI.list.active.appendChild(li);
        });
        
        UI.list.count.textContent = cCount; 
        if(UI.toggleCompletedBtn) UI.toggleCompletedBtn.parentElement.style.display = state.hideCompleted ? 'none' : 'block';
        updateStreaks();
    };
    ...
    document.getElementById('mock-google-sync')?.addEventListener('click', () => { alert('Запуск OAuth 2.0...'); console.log('Mocking Google Workspace Sync flow...'); });
